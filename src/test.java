...
public class test 
{
    ...
    private static void testUnits(ArrayList<UnitData> units)
    {
        ...
        for(int i = 0; i < units.size(); i++)
        {
            ...
            + "     HealthUpgrades Structs: " + units.get(i).HealthUpgradesData.size() + "\n";
        }
    ...
    private static void testBattleWeapon(ArrayList<UnitData> units)
    {
        for (UnitData i : units)
        {
            for(int j = 0; j < i.BattleWeaponData.size(); j++)
            {
                System.out.println(i.BattleWeaponData.get(j).attackName);
                System.out.println(i.BattleWeaponData.get(j).SingleTarget);
            }
        }
    }
}